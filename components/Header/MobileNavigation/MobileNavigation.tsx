        ...
        <div className="flex-1 overflow-y-auto">
          <SheetHeader className="pt-0 px-0">
            <nav className="mt-0 space-y-3">
              {categories.map((item) => (
                <div key={item.name} className={`${openDropdown === item.name ? "bg-[#eff8ff]" : ""} border-t border-[#e5e5e5] py-5 rounded-[0] mb-0`}>
                  {item.subcategories ? (
                    <>
                      <button
                        onClick={() => toggleDropdown(item.name)}
                        className={`flex items-center justify-between w-full text-[#636363] text-[15px] font-bold px-3 ${openDropdown === item.name ? "bg-[#eff8ff] !text-[#004c8f]" : ""}`}
                      >
                        {item.name}
                        <ChevronDown
                      ...
                      {openDropdown === item.name && (
                        <div className="mt-2 ml-0 flex flex-col space-y-2 pt-2 pl-8 pb-3">
                          {item.subcategories.map((child) => (
                            <Link
                              key={child.slug}
                              href={`${process.env.NEXT_PUBLIC_APP_BASE_URL}${item.slug}/${child.slug}`}
                    ...
                    <Link
                      href={`${process.env.NEXT_PUBLIC_APP_BASE_URL}${item.slug}`}
